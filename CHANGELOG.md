...
- 🔒 **Process logs no longer persist secrets in plaintext (homelab#720)** — a command that echoes a secret to stdout (intentionally for debugging, or via an env-var dump) previously got that secret written verbatim into `logs/processes/*.jsonl`, retained on a persistent volume and readable by every future task in the shared pod. This surfaced for real: an audit found a live `GITHUB_APP_PRIVATE_KEY` captured via `env | grep -E '^(GH_|GITHUB_)'`. `BoundedLogWriter.write()` now runs each log line's `data`/`command` fields through best-effort secret-pattern redaction (`open_terminal/utils/redact.py`) before persisting -- GitHub tokens (classic, fine-grained, App/installation), PEM private key blocks, AWS access key IDs, Slack tokens, JWTs, and generic Bearer tokens. This is deliberately not exhaustive (no generic high-entropy heuristics, to avoid false-positive-redacting ordinary output) and can't catch a secret split across two separate 4096-byte PTY read chunks -- best-effort, not a guarantee.
